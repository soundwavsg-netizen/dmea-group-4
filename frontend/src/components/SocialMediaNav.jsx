...
const SocialMediaNav = ({ activeTab, setActiveTab }) => {
    ...
    >
      <div className="w-full px-3 md:px-6 lg:px-8 pl-14 lg:pl-6">
        ...
        >
          ...
          {(isSuperAdmin || permissionsService.canAccessTab('social_media_diagnostics', 'data_input')) && (
            <div
              onClick={() => setActiveTab('data-input')}
              className={tabStyle('data-input')}
              data-testid="social-media-tab-data-input"
            >
              Data Input
            </div>
          )}
          
          {(isSuperAdmin || permissionsService.canAccessTab('social_media_diagnostics', 'column_mapping')) && (
            <div
              onClick={() => setActiveTab('column-mapping')}
              className={tabStyle('column-mapping')}
              data-testid="social-media-tab-column-mapping"
            >
              Column Mapping
            </div>
          )}
          
          {(isSuperAdmin || permissionsService.canAccessTab('social_media_diagnostics', 'dashboard')) && (
            <div
          ...
          {(isSuperAdmin || permissionsService.canAccessTab('social_media_diagnostics', 'insight_summary')) && (
            <div
              onClick={() => setActiveTab('insight-summary')}
              className={tabStyle('insight-summary')}
              data-testid="social-media-tab-insight-summary"
            >
              Insight Summary
            </div>
          )}
